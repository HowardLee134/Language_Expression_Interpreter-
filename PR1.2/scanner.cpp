...
//Project Phase 1.1 (PR1.1): Scanner for expressions
#include <iostream>
#include <string>
#include <regex>
#include <fstream>
...
int main(int argc, char *argv[]) {
    ...
    for(int i=0; i < lineCount; i++){
        ...
        for (int j = 0; j < currLine.length(); ++j) {
            if (currLine[j] != ' ') {
                tokens[i][tokenIndex] = tokens[i][tokenIndex] + currLine[j];
            }
            else {
                tokenIndex++;
            }
        }
        tokensInLine.push_back(tokenIndex + 1);
        tokenIndex = 0;
    }

    //process token
    for(int i = 0; i < lineCount; i++){
        for (int j = 0; j < tokensInLine.at(i); j++) {
